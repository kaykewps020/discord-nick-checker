...
"""
╔══════════════════════════════════════════════════════════════════╗
║     DISCORD RARE NICK CHECKER v6.0 — CHECKPOINT MODE           ║
║                                                                 ║
...
import itertools, datetime, traceback, signal, argparse
...
CONFIG = {
    ...
    "timeout": 15,
    "max_retries": 2,
    "batch_save": 100,          # salva progresso a cada N checks
...
class Checker:
                ...
                try:
                    async with self.session.get(
                        ...
                        if resp.status == 200:
                            ...
                            try:
                                retry = (await resp.json()).get('retry_after', 3)
                            except:
                                retry = 3
                            await asyncio.sleep(retry + random.uniform(0.5, 2))
                            continue
    ...
    async def run(self, combos, start_index, label, progress):
        """Roda scan com checkpoint — salva progresso a cada batch"""
        total = len(combos)
        ...
        async with aiohttp.ClientSession(connector=connector) as session:
            ...
            batch_size = min(CONFIG["max_concurrent"] * 5, 200)
            processed = 0
            ...
            for i in range(0, len(combos_slice), batch_size):
                ...
                eta = (len(combos_slice) - processed) / rate if rate > 0 else 0
                ...
                sys.stdout.write(
                    ...
                    f"{Fore.RESET}| {rate:.0f}/s | ETA: {eta_s}   "
                )
                sys.stdout.flush()
                
                # Salva progresso a cada batch
                if batch_count % 5 == 0 or i + batch_size >= len(combos_slice):
        ...
        print(f"  ⏱ {elapsed//60:.0f}m{elapsed%60:.0f}s | {rate:.0f}/s")
...
async def run_resume():
    ...
    current = progress["current_pattern"]
    start_idx = progress["checked_in_pattern"]
...
def main():
    parser = argparse.ArgumentParser(description="Discord Rare Nick Checker v6.0")
...
╔══════════════════════════════════════════════════════════════════╗
║     ✦ DISCORD RARE NICK CHECKER v6.0 ✦                        ║
║     Endpoint: unauthed | Checkpoint: progress.json             ║
╚══════════════════════════════════════════════════════════════════╝
{Style.RESET_ALL}""")
    
    def signal_handler(sig, frame):
        print(f"\n\n{color('warn', 'Salvando progresso...')}")
        # progress é salvo pelo checker
        sys.exit(0)
